...
int32_t get_baudrate_para(uint8_t port, uint32_t freq, uint32_t *freq_para)
{
  uint32_t AHBClock;
  uint8_t i;
  RCC_ClocksType clockType;
  
  RCC_GetClocksFreq(&clockType);
  AHBClock = clockType.AHBCLK_Frequency;
...
{ 
  ...
  if (SPI_0 == spi->port)
  {
    ...
    init_para.CPHOL = HSPI_SPICR1_CPHOL_LOW;//CPHOL
    init_para.CPHA = HSPI_SPICR1_CPHA_FIRST;//CPHA
  ...
  else if (SPI_1 == spi->port)
  {
    ...
    AltFunIO(SPI1_SSN_Port, SPI1_SSN_Pin, ALTFUN_PULLUP);//SPI_SSN
    ...
    init_para.CPHOL = SPIx_SPICR1_CPHOL_LOW;//CPHOL
    init_para.CPHA = SPIx_SPICR1_CPHA_FIRST;//CPHA
  ...
  else
  {
    ...
    AltFunIO(SPI2_SSN_Port, SPI2_SSN_Pin, ALTFUN_PULLUP);//SPI_SSN
