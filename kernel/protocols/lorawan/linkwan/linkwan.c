...
//static lora_AppData_t tx_data = {tx_buf, LORAWAN_APP_DATA_BUFF_SIZE, 0};
lora_AppData_t tx_data = {tx_buf, LORAWAN_APP_DATA_BUFF_SIZE, 0};
...
static void write_lora_dev(lora_dev_t *lora_dev)
{
    lora_dev->crc = crc16((uint8_t *)lora_dev, sizeof(lora_dev_t) - 2);
	  lora_dev->flag = VALID_LORA_CONFIG;
#ifdef AOS_KV
...
void lora_fsm( void )
{
    ...
    char rx;
				...
				aos_kv_get("lora_dev", &lora_dev, &len);
				if (VALID_LORA_CONFIG == lora_dev.flag)
				{
					memcpy(&g_lora_dev, &lora_dev, sizeof(lora_dev_t));
				}
    ...
    while (1) {
       ...
       run_led_toggle();
        ...
        if (0 == hal_uart_recv(&uart_0,&rx,1,1))
        {
...
#error "Please define a region in the compiler options."
            ...
            case DEVICE_STATE_JOIN: {
                ...
                if (lora_param.JoinMode == JOIN_MODE_OTAA) {
                    ...
                    mibReq.Param.DevAddr = 	(g_lora_abp_id.devaddr[0] << 24) + 
																						(g_lora_abp_id.devaddr[1] << 16) +
																						(g_lora_abp_id.devaddr[2] << 8) +
            ...
            case DEVICE_STATE_JOINED: {
                DBG_LINKWAN("Joined\n\r");
                store_lora_config();
                //device_state = DEVICE_STATE_SEND;//TODO 登陆成功后，进入休眠，不主动发送
                device_state = DEVICE_STATE_SLEEP;
                break;
            }
